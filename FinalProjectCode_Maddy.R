...
WBdata %<>% mutate(Year = as.factor(Year),
                   log.GNI = log(as.numeric(GNI)),
                   log.HealthSpending = log(as.numeric(HealthSpending)),
...
# mutate to make new column - log of Health Spending / log of GNI 
WBdata %<>% mutate(HealthSpendPCNT = log.HealthSpending / log.GNI)
...
datasummary_skim(
  WBdata %>% select(log.GNI, log.HealthSpending, log.DeathRate, log.HIV,log.LifeExpect, log.Malnutrition, 
